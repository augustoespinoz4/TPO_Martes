...
public class Mapa implements MapaTDA {
    ...
    public void calcularCamino(String ciudad1, String ciudad2) {
        ...
        ConjuntoTDA<String> ciudades = mapaCiudades.Ciudades();
        ConjuntoTDA<String> copiaCiudades = copiarConjunto(ciudades);

        while (!copiaCiudades.ConjuntoVacio()) {
            String ciudad = copiaCiudades.Elegir();
            copiaCiudades.Sacar(ciudad);
        ...
        // Guarda la distancia mínima desde ciudad1 a cada ciudad
        DiccionarioSimpleTDA<String, Integer> distanciaMinima = new DiccionarioSimpleDinamico<>();
        distanciaMinima.InicializarDiccionario();

        // Guarda el predecesor de cada ciudad en el camino más corto.
        DiccionarioSimpleTDA<String, String> predecesor = new DiccionarioSimpleDinamico<>();
        predecesor.InicializarDiccionario();
        ...
        while (!ciudades.ConjuntoVacio()) {
            String ciudad = ciudades.Elegir();
            ciudades.Sacar(ciudad);
            distanciaMinima.Agregar(ciudad, Integer.MAX_VALUE); // distancia infinita: aún no se conoce un camino hasta la ciudad
